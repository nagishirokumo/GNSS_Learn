                                                                         ...
                                                                         double Freq_Of_Obs)
{
    ...
    for(auto & [Sys_Flag,Obs_info]:Freq_Observation_Completeness_Rate)
    {
        for(auto & [Obs_Type,Cnt_Value]:Obs_info)
        {
            ...
            printf("Freq_Observation_Completeness_Rate Sys %c Freq %c:%d\n",Sys_Flag,Obs_Type,
                   Cnt_Value.first);
        ...
        for (size_t i = 0; i < vRange.size(); ++i) {
            ...
            while (Epoch_index <= end_index)
            {
                ...
                if (Epoch_index != end_index) {
                    Cycle_Slip_Detection.MW_Diff[prn][Epoch_index]=MW_Value-Observation_Data.Rinex_Obs_Data[Epoch_index+1].Satellites_Obs[prn].Melbourne_Wubbena;
                }
...
void GF_Cycle_Slip_Detection(Rinex304_Observation_Data &Observation_Data,Rinex_Observation_Cycle_Slip_Detection &Cycle_Slip_Detection)
{
    for(auto &[prn,vRange]:Observation_Data.Time_Range)
    {
        ...
        for (size_t i = 0; i < vRange.size(); ++i)
        {
            if(max_len<vRange[i].second-vRange[i].first)
            {
                max_len=vRange[i].second-vRange[i].first;
                max_index=i;
            }
        }
        auto [start_index, end_index] = vRange[max_index];
        //连续观测历元过少不易拟合
        ...
        while(index<=end_index)
        {
            ...
            Cycle_Slip_Detection.GF_Carrier_Phase[prn][index]=
                    Observation_Data.Rinex_Obs_Data[index].Satellites_Obs[prn].Carrier_Phase_Geometry_Free;
            Pseudorange_GF.push_back(Observation_Data.Rinex_Obs_Data[index].Satellites_Obs[prn].Pseudorange_Geometry_Free);
            Cycle_Slip_Detection.GF_Pseudorange[prn][index]=
                    Observation_Data.Rinex_Obs_Data[index].Satellites_Obs[prn].Pseudorange_Geometry_Free;
            ...
            switch (prn[0])
            {
                case 'G':
                {
                    ...
                    &&  std::abs(L_GF3-L_GF2-Q_GF3+Q_GF2)>(C_LIGHT/FREQ_GPS_L2-C_LIGHT/FREQ_GPS_L1) )
                    {
                        ...
                        printf("G Cycle_Slip in Epoch_index:%d %S\n", Epoch_index[j],prn.c_str());
                    }
                    break;
                }
                case 'C':
                {
                    ...
                    {
                        Cycle_Slip_Detection.Cycle_Slip_List[prn].push_back(Epoch_index[j]);
                        printf("C Cycle_Slip in Epoch_index:%.0lf %s\n", Epoch_index[j],prn.c_str());
                    }
                    break;
                }
                case 'R':
...
Rinex_Observation_MultiPath_Detection::Rinex_Observation_MultiPath_Detection(Rinex304_Observation_Data &Observation_Data,double Freq_Of_Obs)
{
    ...
    for(auto &[prn,vRange]:Observation_Data.Time_Range)
    {
        ...
        for (auto &[start_index, end_index] : vRange)
        {
            ...
            while(window_index<= end_index - 49){
                Sum_MP_L1=MultiPath_Freq1_PrefixSum[prn][window_index + 50]-MultiPath_Freq1_PrefixSum[prn][window_index];
                Sum_MP_L2=MultiPath_Freq2_PrefixSum[prn][window_index + 50]-MultiPath_Freq2_PrefixSum[prn][window_index];
                Tmp_L1=0;
                Tmp_L2=0;
                while (index <= window_index + 49)
                {
                    switch (prn[0]) {
                        case 'G': {
                            double MP_L1, MP_L2;
                            ...
                            Tmp_L1 += (MP_L1 - Sum_MP_L1 / 50) *
                                      (MP_L1 - Sum_MP_L1 / 50);
                            Tmp_L2 += (MP_L2 - Sum_MP_L2 / 50) *
                                      (MP_L2 - Sum_MP_L2 / 50);
                            break;
                        }
                        case 'C': {
                ...
                Tmp_L1/=49;
                Tmp_L2/=49;
                MultiPath_Freq1_Assessment[prn][window_index]=std::sqrt(Tmp_L1);
                MultiPath_Freq2_Assessment[prn][window_index]=std::sqrt(Tmp_L2);
                window_index++;
            }
